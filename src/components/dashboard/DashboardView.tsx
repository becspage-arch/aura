...
function parseDayToUTCDate(day: string) {
  // day: "YYYY-MM-DD" -> stable parse (avoid local timezone shift)
...
export default function DashboardView({ clerkUserId }: { clerkUserId?: string }) {
  ...
  const lastTrade =
    state.summary?.status?.lastTradeAt ? fmtTimeLondon(state.summary.status.lastTradeAt) : "—";
  ...
  const monthModel = useMemo(() => {
    ...
    // compute leading blanks (Mon-first)
      ...
      <section className="aura-grid-4">
        ...
        <div className="aura-card">
          ...
          <div className="aura-mini-value">
            {perf?.maxDrawdownUsd != null ? fmtMoneyUsd(perf.maxDrawdownUsd) : "—"}
          </div>
          <div className="aura-stat-sub">Last 30 days</div>
        </div>
      </section>
